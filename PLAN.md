...
### Platform metadata ConfigMap for IRSA (bootstrap identity injection)
- **Decision**: Terraform remains source of truth for AWS IAM roles and Secrets Manager payloads. Bootstrap publishes a cluster-local ConfigMap `rosa-platform-metadata` (namespace `openshift-gitops`) with account/cluster facts and full IRSA ARNs. GitOps charts consume that metadata (Job/hook) instead of hardcoding account-specific ARNs in cluster-config. ESO cannot fetch its own role ARN from SM first (circular); bootstrap breaks the chicken-and-egg.
- **Status**: Accepted.
- **Consequences**:
  - cluster-config stays portable across AWS accounts; per-cluster paths do not embed `arn:aws:iam::ACCOUNT:role/...` for ESO.
...
- **Consequences**:
  ...
  - ESO IRSA ARN is injected via platform metadata (see above), not hardcoded in cluster-config.
