...
- **Consequences**:
  ...
  - Secrets Manager IAM trust includes `external-secrets-operator:external-secrets-sa` (with temporary `openshift-gitops:vplugin` dual-trust during migration).
  - AVP support remains only for one deprecation release window before full removal from bootstrap/plugin paths.
