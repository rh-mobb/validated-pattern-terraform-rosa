...
│   └── configuration.tfvars              # Auto-generated from infrastructure outputs (optional)
...
### `configuration.tfvars` (Optional)

Auto-generated file containing infrastructure outputs passed to the configuration layer. Generated by running `make cluster.<name>.generate-config-tfvars` after infrastructure is applied.

**For CI/CD pipelines**: You can generate this file after infrastructure apply, then use it in the configuration layer:
```bash
# After infrastructure apply
make cluster.<name>.generate-config-tfvars

# Configuration layer will automatically use this file if present
make cluster.<name>.plan-configuration
```

**For local development**: The Makefile automatically extracts infrastructure outputs as environment variables, so this file is optional.
...
- Backend path is automatically set to `clusters/<name>/infrastructure.tfstate` or `clusters/<name>/configuration.tfstate`
...
# Generate configuration.tfvars (optional, for CI/CD)
./scripts/cluster/generate-config-tfvars.sh <cluster-name>

# Initialize configuration
./scripts/cluster/init-configuration.sh <cluster-name>

# Plan configuration
./scripts/cluster/plan-configuration.sh <cluster-name>

# Apply configuration
./scripts/cluster/apply-configuration.sh <cluster-name>
```
...
Egress-zero clusters use a private API endpoint (PrivateLink), so you need a VPN tunnel to access the API. The Makefile automatically starts the tunnel when needed for configuration operations.
...
- Ensure infrastructure has been applied: `make cluster.<name>.apply-infrastructure`
- Generate configuration.tfvars: `make cluster.<name>.generate-config-tfvars`
- Or verify infrastructure outputs are available: `cd terraform/infrastructure && terraform output`
