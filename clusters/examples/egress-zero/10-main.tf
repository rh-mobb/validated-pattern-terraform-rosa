...
module "cluster" {
  ...
  # NOTE: For multi-AZ, ROSA creates "workers-0", "workers-1", "workers-2" (one per subnet)
  machine_pools = [
    {
      name                = "workers" # Must match ROSA's default pool name (will be "workers-0", "workers-1", etc. for multi-AZ)
      instance_type       = var.instance_type
      min_replicas        = 3 # Minimum for HA (multi-AZ)
      max_replicas        = 6 # Double min replicas
      multi_az            = true
