#!/bin/bash
# scripts/cluster/destroy-configuration.sh
# Destroy configuration (with confirmation)
# Usage: destroy-configuration.sh <cluster-name>
...
if [ -z "$API_URL" ]; then
    ...
    warn "Infrastructure may already be destroyed. Attempting to destroy configuration anyway..."
    ...
    # Try to destroy without cluster access
    ...
    if [ -f "$CLUSTER_CONFIG_TFVARS" ]; then
        terraform apply \
            ...
            -auto-approve || (warn "Configuration destroy completed (may have failed if cluster is not accessible)" && exit 0)
    else
        terraform apply \
            -var="enable_destroy=true" \
            -var-file="$CLUSTER_TFVARS" \
...
if [ -z "$ADMIN_PASSWORD" ] && [ -z "${TF_VAR_k8s_token:-}" ]; then
    ...
    warn "Configuration may already be destroyed or infrastructure is missing."
    warn "Attempting to destroy configuration anyway (may fail if cluster is still running)..."
...
if [ -n "$API_URL" ]; then
    ...
    if [ -n "${TF_VAR_k8s_token:-}" ]; then
        ...
        CLUSTER_TFVARS="$CLUSTER_DIR/terraform.tfvars"
        CLUSTER_CONFIG_TFVARS="$CLUSTER_DIR/configuration.tfvars"
        info "Setting enable_destroy=true and applying to remove resources from state..."
        ...
        if [ -f "$CLUSTER_CONFIG_TFVARS" ]; then
            terraform apply \
                ...
                -var-file="$CLUSTER_TFVARS" \
                -var-file="$CLUSTER_CONFIG_TFVARS" \
                -auto-approve || (warn "Configuration destroy completed" && exit 0)
        else
            terraform apply \
                -var="enable_destroy=true" \
                -var-file="$CLUSTER_TFVARS" \
...
success "Configuration destroyed successfully"
