...
export TF_VAR_api_url=$(terraform output -raw api_url 2>/dev/null || echo "")
export TF_VAR_cluster_id=$(terraform output -raw cluster_id 2>/dev/null || echo "")
export TF_VAR_cluster_name=$(terraform output -raw cluster_name 2>/dev/null || echo "")
export TF_VAR_console_url=$(terraform output -raw console_url 2>/dev/null || echo "")

# Optional infrastructure outputs
export TF_VAR_admin_user_created=$(terraform output -raw admin_user_created 2>/dev/null || echo "false")
export TF_VAR_bastion_deployed=$(terraform output -raw bastion_deployed 2>/dev/null || echo "false")
export TF_VAR_bastion_instance_id=$(terraform output -raw bastion_instance_id 2>/dev/null || echo "")
export TF_VAR_bastion_ssm_command=$(terraform output -raw bastion_ssm_command 2>/dev/null || echo "")
export TF_VAR_bastion_sshuttle_command=$(terraform output -raw bastion_sshuttle_command 2>/dev/null || echo "")
