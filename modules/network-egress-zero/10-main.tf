...
resource "aws_security_group" "worker_nodes" {
  ...
  # DNS egress for VPC endpoint DNS resolution
  egress {
...
# Network ACL for strict egress control
resource "aws_network_acl" "private" {
  ...
  # Allow HTTPS outbound to VPC endpoints (VPC endpoint IPs)
  egress {
    rule_no    = 100
    protocol   = "tcp"
    from_port  = 443
  ...
  # Deny all other egress (use maximum rule number 32766, as 32767 is not allowed)
  egress {
    rule_no    = 32766
    protocol   = "-1"
    from_port  = 0
