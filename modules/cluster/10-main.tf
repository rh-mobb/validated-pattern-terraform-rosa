...
resource "rhcs_hcp_machine_pool" "default" {
  ...
  # Reference: https://github.com/rh-mobb/terraform-rosa/blob/main/04-cluster.tf
  replicas = local.autoscaling_enabled ? null : data.rhcs_hcp_machine_pool.default[count.index].replicas
  ...
  aws_node_pool = {
    instance_type            = length(var.machine_pools) > 0 ? var.machine_pools[0].instance_type : data.rhcs_hcp_machine_pool.default[count.index].aws_node_pool.instance_type
  ...
  lifecycle {
    ...
    precondition {
      condition = contains(
        [for mt in data.rhcs_machine_types.available.items : mt.id],
        length(var.machine_pools) > 0 ? var.machine_pools[0].instance_type : data.rhcs_hcp_machine_pool.default[count.index].aws_node_pool.instance_type
      )
      error_message = "Instance type '${length(var.machine_pools) > 0 ? var.machine_pools[0].instance_type : data.rhcs_hcp_machine_pool.default[count.index].aws_node_pool.instance_type}' is not available for ROSA in region '${var.region}'. Use 'terraform console' and run 'data.rhcs_machine_types.available.items' to see available machine types."
    }
  }
}
