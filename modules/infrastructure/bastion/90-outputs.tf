output "bastion_instance_id" {
  description = "Instance ID of the bastion host (for SSM Session Manager access, null if enable_destroy is true)"
  value       = length(aws_instance.bastion) > 0 ? one(aws_instance.bastion[*].id) : null
  sensitive   = false
}

output "bastion_public_ip" {
  description = "Public IP address of the bastion host (only set if bastion_public_ip is true, null if enable_destroy is true)"
  value       = length(aws_instance.bastion) > 0 && var.bastion_public_ip ? one(aws_instance.bastion[*].public_ip) : null
  sensitive   = false
}

output "bastion_private_ip" {
  description = "Private IP address of the bastion host (null if enable_destroy is true)"
  value       = length(aws_instance.bastion) > 0 ? one(aws_instance.bastion[*].private_ip) : null
  sensitive   = false
}

output "bastion_security_group_id" {
  description = "Security group ID of the bastion host (null if enable_destroy is true)"
  value       = length(aws_security_group.bastion) > 0 ? one(aws_security_group.bastion[*].id) : null
  sensitive   = false
}

output "ssh_command" {
  description = "SSH command to connect to bastion (only if bastion_public_ip is true, null if enable_destroy is true)"
  value       = length(aws_instance.bastion) > 0 && var.bastion_public_ip ? "ssh ec2-user@${one(aws_instance.bastion[*].public_ip)}" : null
  sensitive   = false
}

output "ssm_session_command" {
  description = "AWS SSM Session Manager command to connect to bastion (null if enable_destroy is true)"
...
output "sshuttle_command" {
  description = "sshuttle command to create VPN-like access to VPC via bastion (null if enable_destroy is true)"
...
output "ssh_tunnel_command" {
  description = "SSH tunnel command to forward local port 6443 to cluster API via bastion (null if enable_destroy is true)"
  value = length(aws_instance.bastion) > 0 ? (
    var.bastion_public_ip ? (
