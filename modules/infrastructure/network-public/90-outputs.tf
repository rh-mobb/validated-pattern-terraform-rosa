...
output "public_subnet_ids" {
  description = "List of public subnet IDs (empty if enable_destroy_network is true)"
  value       = aws_subnet.public[*].id
  sensitive   = false
}

output "nat_gateway_ids" {
  description = "List of NAT Gateway IDs (one per AZ, empty if enable_destroy_network is true)"
  value       = aws_nat_gateway.main[*].id
  sensitive   = false
}

output "nat_gateway_id" {
  description = "ID of the first NAT Gateway (for backwards compatibility, null if enable_destroy_network is true)"
  value       = length(aws_nat_gateway.main) > 0 ? aws_nat_gateway.main[0].id : null
  sensitive   = false
}

output "internet_gateway_id" {
  description = "ID of the Internet Gateway (null if enable_destroy_network is true)"
...
output "public_subnet_azs" {
  description = "List of availability zones used for public subnets (empty if enable_destroy_network is true)"
