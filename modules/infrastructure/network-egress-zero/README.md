...
This module creates a VPC with private subnets only, VPC endpoints, strict egress controls via security groups, and VPC Flow Logs. Designed for ROSA HCP clusters requiring maximum security with zero internet egress.
...
- **Strict egress control** via security groups (limited egress rules)
...
- **Security Groups**: Limited egress rules (HTTPS 443 and DNS 53 UDP/TCP to VPC CIDR) - all egress must go through VPC endpoints
...
**Note**: Network ACLs are not currently implemented. Access control is managed via security groups only. See TODO for future NACL implementation.
