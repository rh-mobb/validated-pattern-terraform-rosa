...
variable "enable_secrets_manager_iam" {
  description = "Enable Secrets Manager IAM resources for External Secrets Operator IRSA and temporary ArgoCD Vault Plugin (openshift-gitops:vplugin) trust"
  type        = bool
  default     = false
  nullable    = false
}
...
variable "additional_secrets" {
  description = "Additional Secrets Manager secret names for ESO/AVP IAM policy (optional)"
  type        = list(string)
  default     = null
  nullable    = true
}
