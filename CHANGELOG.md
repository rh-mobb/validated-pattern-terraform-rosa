...
- **Built-in OAuth OpenID providers**: add a typed `oidc_identity_providers`
  map, a reusable OIDC identity-provider module, Secrets Manager locator input,
  and Entra ID and Keycloak worked examples with the callback-URL rule, explicit
  state boundaries and each provider's group-claim trap.
...
- **BGP config Secrets Manager secret (#51)**: Route Server module publishes `{cluster}-bgp-config` (`role_arn`, `region`, `route_server_id`, `route_server_ids`) and optionally attaches GetSecretValue on the ESO IAM role. Outputs `bgp_config_secret_name` / `bgp_config_secret_arn`. Virt recipe sets `enable_secrets_manager_iam = true` for GitOps ExternalSecret consumption.
- **Platform metadata ConfigMap for IRSA**: Bootstrap publishes `openshift-gitops/rosa-platform-metadata` (account, region, `secretsManagerRoleArn`, optional `bgpConfigSecretName` / `certManagerRoleArn` / `efsCsiRoleArn` / `efsFileSystemId`) so GitOps charts bind IRSA and EFS CSI without hardcoding account ARNs. ADR: `docs/architecture/platform-metadata-irsa.md`. Bootstrap env exports include `AWS_ACCOUNT_ID`, `SECRETS_MANAGER_ROLE_ARN`, `BGP_CONFIG_SECRET_NAME`, `EFS_CSI_ROLE_ARN`, `EFS_FILE_SYSTEM_ID`. IAM output `efs_csi_role_arn`.
- **OpenShift Virtualization enablement docs**: End-to-end metal Virt + EFS RWX + CUDN BGP / Route Server deploy, GitOps wiring (ESO + `cluster-efs` preferred), and teardown in `docs/deployment/enablement.md`; discoverable from `docs/index.md`, quick start, `README.md`, `PLAN.md`, and `clusters/README.md`. MkDocs Cluster Configurations / Route Server pages use `include-markdown` (not symlinks) so repo-relative links rewrite instead of 404.
- **Registry image mirrors (`image_mirrors`)**: New `modules/infrastructure/cluster/23-image-mirrors.tf` declares one `rhcs_image_mirror` per source repository path, with an `image_mirror_ids` output and a `docs/guides/image-mirrors.md` guide. Lets a cluster redirect image pulls to a reachable mirror, which is what makes third-party operator content installable on a zero-egress cluster — `ImageDigestMirrorSet` objects authored on a hosted-control-plane guest are rejected by admission because the management plane owns that configuration. Digest references only; tag-referenced images are not rewritten. Requires `rhcs >= 1.7.2`, already satisfied by the pinned `~> 1.7.7`.
- **Cluster registry configuration (`registry_config`, default `null`)**: Exposes `additional_trusted_ca`, `registry_sources` (`allowed_registries` / `blocked_registries` / `insecure_registries`), `allowed_registries_for_import` and `platform_allowlist_id` on `rhcs_cluster_rosa_hcp`. `additional_trusted_ca` is what makes a mirror registry behind a private certificate authority usable; without it pulls fail with `x509: certificate signed by unknown authority`. Note that `registry_sources.allowed_registries` is a cluster-wide deny-by-default switch — see `modules/infrastructure/cluster/README.md`.
...
- **Pipeline identity guides** (`docs/guides/pipeline-identities/`): CI cluster identities on both authentication architectures, with a decision page, a shared ServiceAccount path including runnable Jenkins and GitLab pipelines, least-privilege RBAC examples per architecture, and a custody-safe client-credentials helper.
...
### Fixed
- **Route Server destroy blocked by operator BGP peers**: CUDN BGP operator creates `route-server-peers` outside Terraform state, blocking endpoint deletion and `aws_vpc_route_server_vpc_association` disassociation. Add `scripts/cluster/cleanup-route-server-bgp-peers.sh` (manual or pre-destroy), invoke from `destroy-infrastructure.sh` when `enable_route_server=true`, and `null_resource.cleanup_route_server_bgp_peers` in the route-server module (same script on `terraform destroy`).
- **Virt recipe — EFS VM disks and KVM scheduling**: Document that default workers lack `devices.kubevirt.io/kvm` (schedule VMs on `bgp_router` metal pools). `cluster-efs` ≥ 0.5.1 sets EFS access-point `uid`/`gid` **107** on `efs-sc` so KubeVirt disks provision without virt-handler chown failures; add `scripts/cluster/test-virt-efs-live-migrate.sh` smoke test. Companion PRs: `validated-pattern-helm-charts` `cluster-efs` 0.5.1, `rosa-cluster-config` `dev/virt` pin.
...
### Changed
- **OpenShift Virtualization recipe**: Renamed `clusters/bgp` → `clusters/virt` (`cluster_name = "virt"`, `gitops_git_path = "dev/virt"`, `enable_efs = true`). Route Server + metal `bgp_router` pools remain. Make targets are `cluster.virt.*`.
- **Sibling GitOps clones**: Agents must fetch a clean `origin/main` and create a feature branch before editing `reference/validated-pattern-helm-charts` or `reference/rosa-cluster-config` (never commit those remotes — or this repo — on `main`).
- **`cluster-bootstrap` pin 0.5.20**: Bump `helm_chart_version` default and bootstrap script fallback for Virt-ready `efs-sc` uid/gid 107 alignment with `cluster-efs` (validated-pattern-helm-charts #135).
- **BGP operator source**: GitOps `dev/virt` / `cudn-bgp-routing-operator` now builds [bgp-cloud-connector](https://github.com/openshift/bgp-cloud-connector) in-cluster instead of `jingczhang/rosa-bgp-operator`. Docs and Route Server comments updated; Terraform IRSA identity is unchanged.
...
- **Virt recipe worker size**: `clusters/virt` default workers use `m7i.2xlarge` (was `m5.xlarge`) for GitOps/BuildConfig headroom on greenfield applies.
- **Virt GitOps — CDI clone memory**: `dev/virt` pins `rosa-virtualization` **1.0.3** with HyperConverged `storageWorkloads` (4Gi) for CDI clone/upload pods; enablement checklist and [`clusters/virt/AGENTS.md`](clusters/virt/AGENTS.md) updated (companion PRs: validated-pattern-helm-charts, rosa-cluster-config).
