...
- **Route Server variables**: Added `enable_route_server` (bool, default false) and `route_server_asn` (number, default 64512) to root module variables.
...
- **Network module route table ID outputs**: Added `private_route_table_ids` (and `public_route_table_ids` on public networks) for use by the route-server module.
